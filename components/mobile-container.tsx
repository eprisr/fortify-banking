...
export function MobileContainer({ children }: { children: React.ReactNode }) {
	...
	return (
		<MobileContainerCtx.Provider value={container}>
			<div
				ref={ref}
				className="xs:relative xs:mx-auto xs:my-auto flex xs:h-166.75 w-full xs:max-w-93.75 flex-col overflow-hidden pt-6 px-6 pb-9 bg-white xs:rounded-2xl xs:shadow-xl xs:top-1/2 xs:transform-[translateY(10%)]">
				{children}
			</div>
