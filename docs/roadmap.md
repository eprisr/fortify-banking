...
| **Async processing (Upstash QStash)** | ⚪ Planned | *(new, ADR-017 — chosen over the backlog doc's original Redis/BullMQ framing, which needs an always-on worker process that doesn't fit this app's serverless hosting)* Webhook handlers become thin — verify signature, enqueue, return 200 — with the real processing decoupled into a queued HTTP call, so a network drop mid-transfer doesn't corrupt state |
